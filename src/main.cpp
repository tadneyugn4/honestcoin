...
#if defined(NDEBUG)
# error "Honestcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x08d44e294350575c230cda2d31efe26360483329596d1d16c44a0d5c7bc7b1c2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Honestcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Honestcoin Signed Message:\n";
...
{
    // Honestcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Honestcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Honestcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Honestcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Honestcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Honestcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf0df9add62ed2f21ab405bbb8417da81e633485d9477d8bed4e69f0def0e4740");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0417ca30fff0606b4c1306eff4a52b5bbbeb64855184a110b468462a75c644c22d215281a53029c349e638fe19c70fee03d97515ecd69a94e1789b8c2e2fe5d0af") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519212752;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087813886;

        if (fTestNet)
        {
            block.nTime    = 1519212722;
            block.nNonce   = 387022109;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcfd7f88cd6b0fd5c6282097a03d47b1305485ea66b27ed54a17a256fc1fd9e9f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xba, 0xd5 }; // Honestcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HonestcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HonestcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HonestcoinMiner(CWallet *pwallet)
{
    printf("HonestcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("honestcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HonestcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HonestcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HonestcoinMiner, pwallet));
}
